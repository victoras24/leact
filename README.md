...
Coming back to correct some of the above, is not that React runs the cleanup after a re-render or unmount of the component, what React actually does is, before useEffect runs again due to dependency change, React calls the previous effect's cleanup function to clear any outdated logic! The component itself can stay mounted.

Javascript promises

- [ ] Attaching a async into the function, it makes that function to return a promise. That means you will have to await or then to get the result. 
- [ ] One more thing is that promises are actual objects. Funny fact everything in javascript is object. 
- [ ] Creating your own promise might help to build a customised return promise. 
- [ ] Lastly interesting fact is that javascript is a single threaded language thats why you have to await the promises, while the thread responsible for a specific action is busy. 

React mistakes

- [ ] The first one is the style attribute in an html element. The style attribute actually needs double curly brackets lines. But why? Have you ever wondered how it works? The explanation is that in jsx we use curly brackets to run javascript. So opening a curly bracket means we are opening an “expression slot”, weird naming but it makes sense, react team was thinking of the a way that you could express. So, by opening just the curly brackets and adding the styles, is like you want to express an action but you are not. Style attribute in jsx is waiting for an object, unlike in html that is waiting for a string. So that means after opening the expression slot we need to pass an object that will contain the properties in camelCase and the values. This is the explanation of the double curly bracket <p style={{property: value}}. 
- [ ] Another thing is returning multiple top level components. We all know that it needs to be wrapped in a <div> element, but there is a better solution. You could add a <React.Fragment> with the benefit that is not adding a markup. In short, React team thought of the empty element <> to be the React.Fragment.
- [ ] Worth noting, is that by doing setState we are not assigning a value to the state. Remember that states are initiated with const, means that you are not able to change them. Another interesting fact is that when setting a new state, you are not changing the variable, you are scheduling an update! States are working asynchronously. 
- [ ] Have you ever thought why useEffect hooks does not accept async? Here is why. When assigning an async to a function you are making the function return a promise. But the controversy is that useEffects are not meant to return a promise. Are meant to do side effects. 
